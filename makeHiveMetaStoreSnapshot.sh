...
# Creates a mysqldump of your MySQL-backed Hive Metastore.
...
metaStoreHost=hivemetadb.example.com
tsNow=$(date "+%F-%H")
...
snapNow=$targetDir/hiveMetastore-$tsNow.sql.gz
snapLast=$targetDir/hiveMetastore-$tsLast.sql.gz
snapSizeThresh=3
user=user
pass=password
...
function checkforLog () {
   ...
   if [ ! -d $logDir ]; then
      ...
      errSubject="[$(hostname -s)] Hive MetaStore Snapshot Error:  Cannot log backup. ($tsNow)"
      errMessage="Hive MetaStore Error:\n\n\t$logDir doesn't exist"
...
function checkforTarget () {
   ...
   if [ ! -d $targetDir ]; then
      ...
      errSubject="[$(hostname -s)] Hive MetaStore Snapshot Error:  Target directory is missing. ($tsNow)"
      errMessage="Hive MetaStore Snapshot Error:\n\n\t$targetDir is missing"
...
function checkTargetSpace () {
   ...
   if [ $targetSpacePercent -ge $targetSpaceThresh ]; then
      ...
      errSubject="[$(hostname -s)] Hive MetaStore Snapshot Error:  Not enough free space on target. ($tsToday)"
...
function checkIfSnapshotExists () {
   ...
   if [ -s $snapNow ]; then
      ...
      errSubject="[$(hostname -s)] Hive MetaStore Snapshot Error:  Snapshot already exists! ($tsToday)"
...
function compareSnapshotSizes () {
   ...
   if [ $(echo "$percentDiff > $snapSizeThresh" | /usr/bin/bc -l) -eq 1 ]; then
      ...
      errSubject="[$(hostname -s)] Hive MetaStore Snapshot Notice:  Percent change in snapshot size is too great! ($tsNow)"
...
function retireOldSnapshots () {
   ...
   find $targetDir/hiveMetastore* -type f -mtime +7 -exec echo "$(date) retiring backup: " {}  \; >> $logFile
...
function getSnapshot () {
   ...
   mysqldump -C -h $metaStoreHost  -u $user -p$pass hive | gzip > $snapNow
